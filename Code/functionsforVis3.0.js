...
function adjustStatisticData(authorId, type, length)
{
    console.log(author_list);
    var foundIndex = _.findIndex(author_list, function(eachAuthor, index){
    ...
    if(foundIndex != -1)
    {
        if (type === "self")
        {
            console.log("self editing, before:"+author_list[foundIndex].self_edit);
            console.log("length:"+length);
            author_list[foundIndex].self_edit += length;
            console.log("after:" + author_list[foundIndex].self_edit);
        }
        else if (type === "other")
        {
            console.log(author_list[foundIndex]);
            console.log("length:"+length);
            author_list[foundIndex].other_edit += length;
            console.log("after:" + author_list[foundIndex].other_edit);
        }
        else
...
function analyzeEachEditInChangelog(entry, authorId, currentSegID ) 
{
	...
	if (type === 'mlti') {
        ...
        _.find(author_list, function(each)
            {
                if (each.id === authorId)
                    each.only_as = 0;
            });
    ...
    // console.log(effectedSegment);
		...
		if (effectedSegment === undefined) 
		{
			...
			if (segmentsArray.length === 0) 
			{
                ...
                adjustStatisticData(authorId, "self", entryStr.length);
            }
        		...
        		if (startIndex === 0){
        			if (effectedSegment.permanentFlag === true){
                        ...
                        if (effectedSegment.authorId != authorId) {
                            ...
                            adjustStatisticData(authorId, "other", entryStr.length);
                        }
          ...
          effectedSegmentOfDelete = _.find(segmentsArray, function(eachSegment, index) 
          {
            if (eachSegment.sindex === deleteIndex ) 
            {
                    	...
                    	if (eachSegment.authorId != authorId) {
                            ...
                            adjustStatisticData(authorId, "other", 1);
                    	}
                        else if (eachSegment.authorId === authorId)
                        {
                            adjustStatisticData(authorId, "self", 1);
                        }
        	...
        	if(segmentsArray != null )
        	{
        		...
        		if (deleteStartSegmentLocation === deleteEndSegmentLocation) {
                    ...
                    if (effectedSegmentOfDeleteStart.authorId != authorId){
                        ...
                        adjustStatisticData(authorId, "other", deleteEndIndex - deleteStartIndex + 1);    
                    }
                    else if (effectedSegmentOfDeleteStart.authorId === authorId)
                    {
                        adjustStatisticData(authorId, "self", deleteEndIndex - deleteStartIndex + 1);
                    }
        		...
        		// not within one segment, across multiple segments
                    ...
                    if (effectedSegmentOfDeleteEnd.authorId != authorId)
                    {
                        ...
                        adjustStatisticData(authorId, "other", (deleteEndIndex - effectedSegmentOfDeleteEnd.startIndex + 1));    
                    }
                    else if (effectedSegmentOfDeleteEnd.authorId === authorId)
                    {
                        adjustStatisticData(authorId, "self", (deleteEndIndex - effectedSegmentOfDeleteEnd.startIndex + 1)); 
                    }
